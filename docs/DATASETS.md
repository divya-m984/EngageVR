...
## Milestone 5: synthetic feature datasets

Milestone 5 introduces a second kind of dataset: a **windowed feature
dataset** for modelling, described in full in `docs/FEATURE_DATASET.md`.

The only such datasets this repository can currently produce are
deterministic **SYNTHETIC** ones, generated by
`engagevr.features.synthetic` from a documented latent process for the
sole purpose of verifying that the modelling pipeline is correct.

| Property | Value |
|----------|-------|
| Origin | Generated by this software from a seed |
| Subjects | `synthetic-subject-0001`, … — never "participant N" |
| Row labelling | `data_source: synthetic`, `synthetic_label: "SYNTHETIC"` |
| Target labelling | `synthetic_label: "SYNTHETIC"`, `scientific_evaluation_permitted: false` |
| Scientific evaluation | **Prohibited**, schema-enforced |
| Storage | `artifacts/datasets/` (gitignored) |
| Committed | Never |

`baseline-train --mode scientific` refuses any dataset containing a
synthetic row or a target that forbids scientific evaluation.

### No public dataset supplies engagement labels

UBFC-rPPG can validate the *pulse-rate* component only. It carries no
engagement or cognitive-load annotation, and no adapter in this repository
produces one. There is currently **no public dataset adapter that could
supply a target** for the modelling pipeline, which is why
`public_dataset_annotation` exists in the target source vocabulary and is
empty.

### Current status

**No real dataset — public or participant — has been used to fit or
evaluate any model in this repository.** No accuracy, F1, MAE, RMSE, R²,
Brier, or calibration figure against real data exists here, and none will
be reported until the pipeline has been run against real data with real,
documented labels.
